...
public class MainActivity extends AppCompatActivity {
  ...
  static final double MIN_OPENGLES_VERSION = 2.0;
  ...
  private static final class ReleaseConfig {
    ...
    int[] maxOutputImageSizePx;

    ReleaseConfig(RELEASE_TYPE releaseType, final int[] maxOutputImageSizePx) {
      if (maxOutputImageSizePx.length != 2) {
        throw new IllegalArgumentException("Max. image size must have 2 elements but it has "
            + maxOutputImageSizePx.length);
      }
      this.releaseType = releaseType;
      this.maxOutputImageSizePx = new int[]{maxOutputImageSizePx[0], maxOutputImageSizePx[1]};
    }
  }
  ...
  private void initUi() {
    ...
    editTextDstCamImgSizePxX.addTextChangedListener(new TextWatcher() {
      ...
      public void onTextChanged(CharSequence s, int start, int before, int count) {
        ...
        if (s.length() > 0) {
          ...
          if (newDstCamImgSizeX > 0 && newDstCamImgSizeX < RELEASECONFIG.maxOutputImageSizePx[0]) {
        ...
        if (needToReset) {
          //((EditText) findViewById(R.id.editTextDstCamImgSizePxX)).setText(
          //    Integer.toString(mDstCamImgSizePx[0]));
        }
      }
    });
    ...
    editTextDstCamImgSizePxY.addTextChangedListener(new TextWatcher() {
      ...
      public void onTextChanged(CharSequence s, int start, int before, int count) {
        ...
        if (s.length() > 0) {
          ...
          if (newDstCamImgSizeY > 0 && newDstCamImgSizeY < RELEASECONFIG.maxOutputImageSizePx[1]) {
        ...
        if (needToReset) {
          //((EditText) findViewById(R.id.editTextDstCamImgSizePxY)).setText(
          //    Integer.toString(mDstCamImgSizePx[1]));
        }
      }
    });
    seekBarSrcCamHfovDeg.setProgress(valueScaleToSeekBarScale(seekBarSrcCamHfovDeg, mSrcCamHfovDeg,
        SRCCAM_HFOVDEG_MAX));
    seekBarSrcCamFocalLength.setProgress(valueScaleToSeekBarScale(seekBarSrcCamFocalLength,
        mSrcCamFocalLength, SRCCAM_FOCALLENGTH_MAX));
    seekBarSrcCamOpticalCenterX.setProgress(valueScaleToSeekBarScale(seekBarSrcCamOpticalCenterX,
        mSrcCamOpticalCenter[0], SRCCAM_OPTICALCENTER_1DIM_MAX));
    seekBarSrcCamOpticalCenterY.setProgress(valueScaleToSeekBarScale(seekBarSrcCamOpticalCenterY,
        mSrcCamOpticalCenter[1], SRCCAM_OPTICALCENTER_1DIM_MAX));
    seekBarDstCamHfovDeg.setProgress(valueScaleToSeekBarScale(seekBarDstCamHfovDeg, mDstCamHfovDeg,
        DSTCAM_HFOVDEG_MAX));
    editTextDstCamImgSizePxX.setText(String.format(Locale.US, "%d", mDstCamImgSizePx[0]));
    editTextDstCamImgSizePxY.setText(String.format(Locale.US, "%d", mDstCamImgSizePx[1]));
    ...
    Log.d(TAG, "mSrcCamHfovDeg, seekBarSrcCamHfovDeg: " + mSrcCamHfovDeg + ", "
        + seekBarSrcCamHfovDeg.getProgress());
    Log.d(TAG, "mSrcCamFocalLength, seekBarSrcCamFocalLength: " + mSrcCamFocalLength + ", "
        + seekBarSrcCamFocalLength.getProgress());
    Log.d(TAG, "mDstCamHfovDeg, seekBarDstCamHfovDeg: " + mDstCamHfovDeg + ", "
        + seekBarDstCamHfovDeg.getProgress());
  }
  ...
  private void setUiComponentsVisibilityAccordingToReleaseType() {
    ...
    switch (RELEASECONFIG.releaseType) {
      case FREE:
        findViewById(R.id.textViewSrcCamOpticalCenterX).setVisibility(View.GONE);
        findViewById(R.id.seekBarSrcCamOpticalCenterX).setVisibility(View.GONE);
        findViewById(R.id.textViewSrcCamOpticalCenterY).setVisibility(View.GONE);
        findViewById(R.id.seekBarSrcCamOpticalCenterY).setVisibility(View.GONE);
        break;
      case PRO:
        break;
    }
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent intent) {
    ...
    if (resultCode == Activity.RESULT_OK && requestCode == LOADIMAGEFROMGALLERY_REQUESTCODE) {
      try {
        ...
        Log.d(TAG, "mSrcImageUri: " + mSrcImageUri);
        mSrcImage = MediaStore.Images.Media.getBitmap(this.getContentResolver(), mSrcImageUri).copy(Bitmap.Config.ARGB_8888, false);
        Log.d(TAG, "mSrcImage size in onActivityResult(.)...: " + mSrcImage.getWidth() + "x" + mSrcImage.getHeight());
        if (mSrcImage.getWidth() <= RELEASECONFIG.maxOutputImageSizePx[0]
            && mSrcImage.getHeight() <= RELEASECONFIG.maxOutputImageSizePx[1]) {
          //final float dpi = getResources().getDisplayMetrics().density;
          final float dpi = 1.0f; // From gallery the bitmap size tells the actual image resolution, don't know why
          ((EditText) findViewById(R.id.editTextDstCamImgSizePxX)).setText(
              String.format(Locale.US, "%d", (int) ((float) mSrcImage.getWidth() / dpi)));
          ((EditText) findViewById(R.id.editTextDstCamImgSizePxY)).setText(
              String.format(Locale.US, "%d", (int) ((float) mSrcImage.getHeight() / dpi)));
          Log.d(TAG, "mDstCamImgSizePx in onActivityResult(.): " + mDstCamImgSizePx[0] + "x" + mDstCamImgSizePx[1]);
          updateImages();
        } else {
          Toast.makeText(this, "Output image must be max. "
              + RELEASECONFIG.maxOutputImageSizePx[0] + " pixels wide and max. "
              + RELEASECONFIG.maxOutputImageSizePx[1] + " pixels high", Toast.LENGTH_LONG).show();
        }
      } catch (Exception e) {
        Log.e(TAG, e.getMessage() != null ? e.getMessage() : "Failed to load image");
      }
    }
  }
